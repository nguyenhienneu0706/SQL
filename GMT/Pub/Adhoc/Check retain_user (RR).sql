-- Check thông tin user quay trở lại:
...
B AS 
(
SELECT distinct (user_id) AS retain_user, media_source, country_code, campaign_type, campaign, device_id
FROM
  `gamotasdk5.bidata.appsflyer_login_postback`
WHERE game_id = '180941' and app_id = 212251
AND Date = "2023-10-17"
) 
SELECT distinct (A.user_id), B.retain_user, B.media_source, B.country_code, B.campaign_type, B.campaign, B.device_id
FROM A 
LEFT JOIN B 
ON A.USER_ID = B.retain_user
