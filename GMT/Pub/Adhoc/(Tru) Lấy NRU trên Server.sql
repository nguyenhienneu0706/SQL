...
SELECT COUNT(DISTINCT Raw.user_id) AS NRU, Raw.server_id, Server_OD, Server_CD, first_date
FROM
(
...
WHERE first_date >= Server_OD AND first_date < Server_CD -- ĐIỀU KIỆN NRU: USER ĐĂNG KÍ TK & VÀO SERVER TRONG KHOẢNG THGIAN NÀY (k nhất thiết phải group by theo first_date)
) 
AS Raw
WHERE first_date >= Server_OD AND first_date < Server_CD -- ĐIỀU KIỆN NRU: USER ĐĂNG KÍ TK & VÀO SERVER TRONG KHOẢNG THGIAN NÀY (k nhất thiết phải group by theo first_date)
